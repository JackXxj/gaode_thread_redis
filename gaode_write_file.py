...
def gaode(lock, r, first_hour, fileout):
    while True:
        try:
            current_hour = time.strftime('%H')
            if current_hour != first_hour:  # 退出进程
                print time.strftime('[%Y-%m-%d %H:%M:%S]：gaode()退出')
                return False
            if time.strftime('%M%S') == '0000':
                print time.strftime('当前时间：[%Y-%m-%d %H:%M:%S]')
            data = r.lpop('spider:python:gaode:keyword:dest')
            if data:
                with lock:
                    fileout.write(data)
                    fileout.write('\n')
...
def main():
    ...
    print time.strftime('[%Y-%m-%d %H:%M:%S]'), 'redis中gaode的数据量：', gaode_length
    lock = threading.Lock()
